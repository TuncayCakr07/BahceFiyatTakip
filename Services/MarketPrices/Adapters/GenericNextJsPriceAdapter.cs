...
/// <summary>
/// Next.js tabanlı marketler için generic adapter.
/// Veriyi __NEXT_DATA__ script bloğundan veya JSON state'den çıkarır.
/// </summary>
public sealed partial class GenericNextJsPriceAdapter(
    HttpClient httpClient,
    ILogger<GenericNextJsPriceAdapter> logger)
{
    private const string AdapterProvider = "GenericNextJs";
    ...
    {
        logger.LogInformation("GenericNextJs [{Market}]: Direkt URL → {Url}", market.Name, directUrl);

        var html = await FetchAsync(directUrl, market.BaseUrl, ct);
        if (html is null) return null;

        var candidates = ParseNextJsData(html, directUrl);
        ...
        CancellationToken ct = default)
    {
        ...
        var candidates = ParseNextJsData(html, searchUrl);
        if (candidates.Count == 0) return null;

        return PickBest(candidates, market, productName, varietyName, varietyId, productUnit);
    }

    private static List<NextJsCandidate> ParseNextJsData(string html, string sourceUrl)
    {
        var results = new List<NextJsCandidate>();

        // 1. __NEXT_DATA__ script bloğu
        ...
        // 2. Window.NEXT_DATA veya benzeri JSON state blokları (Regex ile)
        foreach (Match block in StateJsonRegex().Matches(html))
        {
            var json = WebUtility.HtmlDecode(block.Groups["json"].Value).Trim();
            WalkJson(json, sourceUrl, results);
        }
    ...
    private static void WalkJson(string json, string sourceUrl, List<NextJsCandidate> results)
    {
        try
        {
    ...
    private static void WalkElement(JsonElement el, string sourceUrl, List<NextJsCandidate> results, int depth)
    {
        if (depth > 10) return;
    ...
    private static NextJsCandidate? TryBuildCandidate(JsonElement el, string sourceUrl)
    {
        // İsim adayları
        ...
        // Fiyat adayları
        decimal price = 0;
        foreach (var key in new[] { "price", "salePrice", "discountedPrice", "unitPrice", "amount", "value" })
        {
        ...
        // Stok
        bool? inStock = GetBool(el, "inStock", "available", "isAvailable", "hasStock");

        // URL
        var url = GetStr(el, "url", "productUrl", "slug", "link") ?? sourceUrl;
        if (!url.StartsWith("http")) url = $"https://{url}"; // Basit slug handle

        // Resim
        var img = GetStr(el, "imageUrl", "image", "thumbnail");

        return new NextJsCandidate(name, price, url, img, inStock);
    }
    ...
    private MarketPriceResult? PickBest(
        List<NextJsCandidate> candidates,
        Market market,
    ...
    [GeneratedRegex(@"window\.(?:__NEXT_DATA__|NEXT_STATE|APP_STATE)\s*=\s*(\{[\s\S]*?\});", RegexOptions.IgnoreCase)]
    private static partial Regex StateJsonRegex();

    private sealed record NextJsCandidate(string Name, decimal Price, string? Url, string? ImageUrl, bool? InStock);
}
